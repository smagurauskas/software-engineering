...
Code review is an important step of the overall software development process. 

Research published by Microsoft found that developers spend ~6 hours per week preparing code for PRs or reviewing others' PRs, amounting to around **~15% of total work time** being related to the code review process. [^1]
...
Modern code reviews use tools to inspect changes and leave comments or start discussions regarding these changes. This is stated in contrast to older scientific articles, where code review process can mean very formal, in-person code review, where several people are looking at printouts of the code.

After reviewing the changes, reviewers leave comments on the code reviewed.
...
{
    public string FirstName { get; set; }
    public string Code { get; set; } // -- What kind of code is this? 
    // Maybe personal identification code, but very ambigious just from the name.
}
```
:::
:::
...
```csharp
class U // -- Cannot understand the intent of the class from it's name. 
// There is also no state, it could probably be `static`
{
    ...
    { // -- What if the list is empty? It is also never modified in the method.
        ...
        foreach (var i in l) s += i;
        return s / l.Count; // -- Division results in `double` result, 
        // but it is narrowed down to `int` by the method. Is this intended?
    }
}
```
...
```csharp
class OrderProc // -- This class has multiple responsibilities: file I/O and pricing logic. 
// Need to be able to test one without the other.
{
...
class ShippingCostCalculatorV1
{
    ...
    {
        var baseCost = 5m;
        if (country == "US") baseCost = 3m; // -- Since it clearly changes based on country, 
        // maybe this can be placed elsewhere for clarity?
        if (weightGrams > 1000) baseCost += 4m;
        if (country == "DE" && weightGrams < 500) 
        // -- It would be clearer if country specific logic would be separated.
            baseCost -= 1m;

...
Web is based on a client-server architecture ^[HTTP/3 has mixed thing up a bit by allowing the server to push resources to the client once the connection is established.]:
...
When looking at the definition of "service" from a systems perspective, we can say that it is something that can either:
...
An HTTP API can be loosely defined as a program that works over HTTP(s) and can be interacted with using machine-readable content types (formats).
...
HTTP APIs have endpoints, which are defined by URLs.

A URL defines a specific resource and how to access it. Any website address is an URL. 
...
In practice, you almost never form the HTTP requests manually, but rather use some library which abstracts most of the internals.
...
HTTP methods (aka *verbs*) identify actions that should be performed on the URL (resource). Each action has a canonical meaning that the method should do.
...
::: {.notes}
See more in [tag popularity in stack overflow questions](https://trends.stackoverflow.co/?tags=json,xml,yaml) ^[old remark, stackoverflow is likely no longer relevant for anything :)]
:::
...
::: {.fragment}
Popular for machine-to-machine communication.
:::
